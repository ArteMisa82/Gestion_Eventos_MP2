...
const UTA_LOGO_PATH = path.join(__dirname, 'UTA.png');
const FISEI_LOGO_PATH = path.join(__dirname, 'FISEI.jpeg');

// --- Datos de Texto ---
const NOMBRE_UNIVERSIDAD = 'UNIVERSIDAD TÉCNICA DE AMBATO';
const NOMBRE_FACULTAD = 'FACULTAD DE INGENIERÍA EN SISTEMAS ELECTRÓNICA E INDUSTRIAL';
const NOMBRE_CARRERA = 'CARRERA DE TECNOLOGÍAS DE LA INFORMACIÓN';
const FIRMA_COORDINADOR_TEXT = 'Firma del Coordinador';

// --- Interfaces ---
interface UsuarioData { nom_usu: string; ape_usu: string; ced_usu: string | null; }
interface EventoData { nom_evt: string; fec_evt: Date; cos_evt: string; }
interface DetalleEventoData { hor_det: number; not_evt_det: number | null; asi_evt_det: number | null; eventos: EventoData; }
interface RegistroEventoData { detalle_eventos: DetalleEventoData; }
...
export class CertificadoService {
    ...
    async getCertificateData(registrationId: number): Promise<CertificateData> {
        ...
        if (!data) throw new Error(`Registro de persona ID ${registrationId} no encontrado.`);
    ...
    async generateCertificate(data: CertificateData): Promise<Buffer> {
        ...
        const eventoEsGratuito = !costoLimpio || costoLimpio.toUpperCase() === 'GRATUITO' || costoLimpio === 'GRATIS' || costoLimpio === '0';
        
        if (!eventoEsGratuito) { 
            const pagoCompleto = data.pagos.some((p: { pag_o_no: number | null }) => {
                const estadoPago = p.pag_o_no;
                return estadoPago !== null && estadoPago !== undefined && parseInt(estadoPago.toString()) === 1;
            });

            if (!pagoCompleto) {
                throw new Error('El participante no ha completado el pago requerido para el certificado.');
            }
        }
        ...
        const notaAprobada = detalle.not_evt_det !== null && detalle.not_evt_det >= NOTA_MINIMA;
        ...
        if (!esAptoParaCertificado) {
            ...
            if (detalle.not_evt_det !== null && !notaAprobada) {
                ...
                razon = `la Asistencia (${(asistenciaActual * 100)}%) es menor al ${ASISTENCIA_MINIMA * 100}% requerido.`;
            }
        ...
        return new Promise((resolve, reject) => {
            ...
            const eventDate = new Date(detalle.eventos.fec_evt).toLocaleDateString('es-ES', {
            ...
            doc.image(FISEI_LOGO_PATH, pageWidth - margin - logoWidth, margin, { width: logoWidth });
            ...
            doc.font('Helvetica-Bold')
                .fontSize(28)
                .fillColor('#333')
                .text('CERTIFICADO DE PARTICIPACIÓN', 0, doc.y + 20, { align: 'center' })
                .moveDown(2);
            ...
            doc.font('Helvetica')
                .fontSize(14)
                .text(`Por haber participado en el evento: ${eventName}`, { align: 'center' })
                .text(`Con una duración de ${eventHours} horas académicas.`, { align: 'center' })
                .text(`Realizado el ${eventDate}.`, { align: 'center' })
                .moveDown(2);
            
            if (detalle.not_evt_det !== null || detalle.asi_evt_det !== null) {
                const nota = detalle.not_evt_det !== null ? `Nota: ${detalle.not_evt_det}` : '';
                const asistencia = detalle.asi_evt_det !== null ? `Asistencia: ${(detalle.asi_evt_det * 100).toFixed(2)}%` : '';
